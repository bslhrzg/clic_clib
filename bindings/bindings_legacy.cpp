...
m.doc() = R"doc(
    ...
    py::class_<HamiltonianTables>(m, "HamiltonianTables", "Pre-computed tables for efficient H|psi> application.")
        .def(py::init<>())
        .def("__repr__", [](const HamiltonianTables& sh) {
            return "<HamiltonianTables for K=" + std::to_string(sh.n_spin_orbitals) + " spin-orbitals>";
        });
    ...
    m.def("apply_hamiltonian",
        [](const Wavefunction& psi, const HamiltonianTables& sh, py::array H, py::array V, double tol_element) {
    ...
    m.def("build_fixed_basis_tables",
        ...
        This function takes:
          * `screened_full`: the full screening tables (from build_hamiltonian_tables)
        ...
        The resulting HamiltonianTables is typically much smaller and allows
